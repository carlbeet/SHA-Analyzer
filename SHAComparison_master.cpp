...
#define f1(a) (ROTR(a, 2) ^ ROTR(a, 13) ^ ROTR(a, 22)) //ep0 //the solution of which is the difference of the two being compared
...
int main()
{
    // map representing all of the input passwords, with first element being username (numbered 1 to about 100,000) and second element being password
    ...
    // calculate how much time it took to run SHA-1 algorithm for entire passwords map
    auto startTime1 = chrono::steady_clock::now();
    map<string, string> sha_1 = SHA1(passwords);
    auto endTime1 = chrono::steady_clock::now();
    auto duration1 = chrono::duration<double>(endTime1 - startTime1);
    cout << "SHA-1 Duration: " << duration1.count() << " seconds." << endl;

    // calculate how much time it took to run SHA-2 algorithm for entire passwords map
    auto startTime2 = chrono::steady_clock::now();
    map<string, string> sha_2 = SHA2(passwords, RoundConstants);
    auto endTime2 = chrono::steady_clock::now();
    auto duration2 = chrono::duration<double>(endTime2 - startTime2);
    cout << "SHA-2 Duration: " << duration2.count() << " seconds." << endl;
...
map<string, string> SHA1(map<string, string>& passwords)
{
    ...
    for (auto iter = passwords.begin(); iter != passwords.end(); iter++)
    {
        ...
        cout << "encrypted: " << encrypted[iter->first] << endl;
    }
    return encrypted;
}
...
{
    //overloaded method, this one is int to hex
    // input: int number
    // output: string of hex value
    //any typecasting should go here.
    int n = number;
    int temp;
    string hex = "";
    vector<char> arr;
...
string binarytoHex(string binary_string)
{ 
    //pass around binary as strings, convert to and from bitset.
    // input: binary string
    // output: string of hex value 
